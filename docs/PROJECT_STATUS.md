...
**Phase 11 — Notifications & Monitoring** (IN PROGRESS — increments 1–13 done)
...
- Increment 12: strategy-vs-strategy comparison (2026-09-19)
  - Head-to-head view: `GET /api/analytics/strategies/compare` returns one FIFO summary per
    strategy the requesting user runs — `{ strategyId, botIds, symbols, metrics }` rows, sorted by
    net P&L (ties by strategy id) so ordering is deterministic
  - Reuses the same `strategyAggregate` engine as `strategyTradeAnalytics` (rolls FIFO round trips
    across the user's own bots); every bot set is derived server-side from the user's bots
    (AGENTS.md §23) — no ids arrive from the client and cross-user data can never leak in
  - Web `/analytics` gains a "Strategy comparison" table (Strategy / Bots / Symbols / Trades /
    Win rate / Net P&L / Profit factor), best net P&L first; empty state when no bots run
  - Unit tests: api **356** (+3 comparison; **622** total), web typecheck/lint/`next build` pass;
    no schema change
- Increment 11: sub-day (hourly) period analytics (2026-09-19)
  - Extends the performance report's per-period table below a day: `periodReturns(curve, 'hour')`
    now buckets the equity curve by UTC hour (skipped hours chain from the last close — nothing is
    fabricated, AGENTS.md §27)
  - `PeriodGranularity` gains `'hour'`; hour start/next/label helpers truncate to the UTC hour
    (`yyyy-mm-ddThh:00`)
  - `PerformanceReport.periodReturns` adds `hourly` to the payload; the CSV export emits hourly rows
    before daily/weekly/monthly
  - Web `/analytics` "Period returns" section gains an "Hourly" table (same null-return `—` and tone
    rendering as day/week/month)
  - Unit tests: api **353** (+2 periodReturns hourly; **619** total), web
    typecheck/lint/`next build` pass; no schema change
- Increment 10: live-account portfolio equity curve (2026-09-19)
  - The broker only ever reports a point-in-time balance; this increment persists it on a recurring
    tick so the live account gets a real equity curve and metrics exactly like the paper one
  - Additive migration `1700000000012-AddLivePortfolioSnapshots`: new `live_portfolio_snapshots`
    table (FK to `paper_accounts` ON DELETE CASCADE, `equity`/`cash`/`position_value`/`realized_pnl`
    numerics, timestamps) + `(account_id, created_at)` index — mirrors the paper snapshot table
  - `LivePortfolioSnapshotEntity` + `LivePortfolioRepository` (abstract port + TypeORM impl) in the
    live-trading module; `LiveTradingModule` registers and exports the repository
  - `LivePortfolioService.recordSnapshot/recordAll`: sums broker-observed balances straight from
    `adapter.getAccountState()` (`total` → equity, `free` → cash; AGENTS.md §13/§14), skips
    unreadable brokers (`LIVE_PORTFOLIO_SNAPSHOT_SKIPPED`, logged, never fabricated) and returns
    null without a configured adapter; `positionValue`/`realizedPnl` stay `'0'` (not yet derivable
    from spot balances) — the equity curve only reads `equity`
  - `LivePortfolioScheduler` records a snapshot every 60s (unref'd timer, gated on
    `isLiveConfigured()` — never runs in paper-only setups), scoped to accounts with live activity
  - `AnalyticsService.livePortfolioAnalytics` (ownership-scoped via `findAccount`, 400 without a
    live broker) reuses `equityCurve`/`metricsFromCurve` over the live snapshots; new
    `GET /api/analytics/live/:accountId/portfolio` (JWT-gated); `AnalyticsModule` imports
    `LiveTradingModule` (no cycle)
  - Web `/analytics`: "Live portfolio" section — equity/peak/return/max-drawdown/starting-cash
    metric cards plus a live equity curve chart; starting cash is the earliest snapshot's cash
  - Unit tests: api **351** (+2 repo, +5 service, +4 analytics; **617** total), web
    typecheck/lint/`next build` pass; migration SQL reviewed (no database available to apply it)
- Increment 9: live trade analytics (2026-09-19)
  - FIFO parity for the live broker path: same `reconstructTrades`/`summarizeTrades` engine as
    increments 3/7/8, now over the active provider's fills on an account (paper fills excluded)
  - New `PaperOrderRepository.listFilledByAccountAndProvider` (`account_id` + `provider` + positive
    fill, ascending); live fills live in the same ledger with `provider = 'binance' | 'bybit'`
  - `AnalyticsService.liveTradeAnalytics` validates account ownership, resolves the active provider
    server-side via `BrokersService` (400 when none configured) and returns `provider`,
    `environment`, FIFO `metrics` and recent round trips
  - New `GET /api/analytics/live/:accountId/trades` (JWT-gated); `AnalyticsModule` imports
    `BrokersModule` (no cycle — Brokers imports only Users)
  - Web `/analytics`: "Live trade analytics" section (account-scoped) showing provider + environment,
    metric cards and a trades table; shared `TradeMetricCards`/`TradesTable` components now replace
    the three duplicated metric grids and trade tables (account/bot/strategy)
  - Unit tests: api **340** (+1 repo, +4 service; **606** total), web typecheck/lint/`next build`
    pass; no schema change
- Increment 8: per-strategy trade analytics (2026-09-19)
  - Rolls FIFO round trips up across every bot of the requesting user that shares one `strategyId`
    (all accounts/symbols), so a strategy's aggregate performance is visible even when it runs on
    several bots
  - New `PaperOrderRepository.listFilledByBotIds` (empty set short-circuits; `bot_id IN (...)` +
    positive fill, ascending); the bot set is always derived server-side from
    `BotRepository.listByUserId` + an in-service filter on `strategyId`, so ownership is enforced
    without trusting the path param (AGENTS.md §23)
  - New `GET /api/analytics/strategies/:strategyId/trades` (JWT-gated, 404 when the user has no bot
    running the strategy) returning `strategyId`, `botIds`, `symbols`, FIFO `metrics` and recent
    round trips — same FIFO engine as increments 3/7
  - Web `/analytics`: "Strategy trade analytics" section worldwide (not account-scoped) — strategy
    dropdown derived from the user's bots with bot counts; metric cards + recent-trades table
  - Unit tests: api **335** (+2 repo, +4 service; **601** total), web typecheck/lint/`next build`
    pass; no schema change (reuses increment 7 attribution)
- Increment 7: per-bot trade analytics (2026-09-19)
...
- Not yet: overlapping-window equity-return comparison across strategies (account curves are
  account-scoped, not strategy-scoped, so returns can only be compared when the strategies share an
  account curve window)

---

### Phase 11 — Notifications & Monitoring (IN PROGRESS)

- Increment 13: in-app notification center (2026-09-19)
  - New `notifications` module (PostgreSQL-backed — never Redis, AGENTS.md §13): migration `…013`
    creates `notifications` (user_id FK users ON DELETE CASCADE, type, severity, title, body, link,
    read_at, created_at; index `idx_notifications_user_read_created`)
  - `NotificationsService` is exported so emitters can write rows; writes are internal-only — the
    controller (read/list + mark read/all-read, user-scoped under JWT, AGENTS.md §23) can never
    forge entries
  - Bot lifecycle emits in `BotsService` after each successful state transition: `BOT_STARTED`,
    `BOT_PAUSED`, `BOT_RESUMED`, `BOT_STOPPED` (info) and `CIRCUIT_BREAKER_TRIGGERED` (error) on
    emergency stop — never for a state-machine rejection (AGENTS.md §19, §25)
  - Web: `/notifications` page (severity dots, mark-read, mark-all-read, unread count) + dashboard
    link; no unread badge yet (no shared navbar exists)
  - Unit tests: api **371** (+15; **637** total), web typecheck/lint/`next build` pass; additive
    migration only
...
| `/notifications`  | Functional             | In-app notification center (read/read-all, severity tones)         |
...
1. **Phase 11 — Increment 13: in-app notification center (2026-09-19)** — additive migration
   `…013` `notifications` table (user_id, type, severity, title, body, link, read_at, created_at;
   index `idx_notifications_user_read_created`). New `notifications` module: writes are
   internal-only via the exported `NotificationsService`; the JWT-guarded controller lists (unread
...
/api/analytics/strategies/compare` returns one FIFO summary per strategy the requesting user
   ...
   `strategyAggregate` engine with `strategyTradeAnalytics`; bot sets are derived server-side from
   the user's own bots (AGENTS.md §23). Web `/analytics` gains a "Strategy comparison" table
   (Strategy / Bots / Symbols / Trades / Win rate / Net P&L / Profit factor) with an empty state.
   No schema change. Unit tests: api **356** (**622 total**), web typecheck/lint/`next build` pass.
3. **Phase 9 — Increment 11: sub-day (hourly) period analytics (2026-09-19)** — extended
   `periodReturns(curve, granularity)` with the `'hour'` granularity (`PeriodGranularity` +
   hour-start/next/label helpers truncate to the UTC hour, `yyyy-mm-ddThh:00` labels; skipped hours
   chain from the last close so nothing is fabricated). `PerformanceReport.periodReturns` gains an
   `hourly` list, the CSV export emits hourly rows before daily/weekly/monthly, and the web
   `/analytics` "Period returns" section adds an "Hourly" table with the same null-return `—` and tone
   rendering. No schema change. Unit tests: api **353** (**619 total**), web
   typecheck/lint/`next build` pass.
4. **Phase 9 — Increment 10: live-account portfolio equity curve (2026-09-19)** — additive migration
   `1700000000012-AddLivePortfolioSnapshots` (new `live_portfolio_snapshots` table + index) plus
   `LivePortfolioSnapshotEntity`/`LivePortfolioRepository` (abstract + TypeORM) in the live-trading
   module. `LivePortfolioService.recordSnapshot/recordAll` sum broker-observed balances straight from
   `adapter.getAccountState()` (total → equity, free → cash), skip unreadable brokers without
   persisting anything, and return null with no adapter; `positionValue`/`realizedPnl` stay `'0'` (not
   yet derivable from spot balances). `LivePortfolioScheduler` records every 60s, gated on a
   configured live broker. `AnalyticsService.livePortfolioAnalytics` reuses `equityCurve`/
   `metricsFromCurve` over the live snapshots; new `GET /api/analytics/live/:accountId/portfolio`
   (JWT-gated, 400 without a live broker); `AnalyticsModule` imports `LiveTradingModule` (no cycle).
   Web `/analytics` adds the "Live portfolio" section (metric cards + live equity curve chart). No
   strategy/risk/broker changes. Unit tests: api **351** (**617 total**), web
   typecheck/lint/`next build` pass; migration SQL reviewed (no database available here to apply it).
5. **Phase 9 — Increment 9: live trade analytics (2026-09-19)** — FIFO parity for the live broker
   path: `PaperOrderRepository.listFilledByAccountAndProvider` (account + provider + positive fill,
   ascending) feeds new `AnalyticsService.liveTradeAnalytics`, which validates account ownership,
   resolves the active provider via `BrokersService` (400 when none configured) and returns the
   provider, environment, FIFO metrics and recent round trips. New
   `GET /api/analytics/live/:accountId/trades` (JWT-gated); `AnalyticsModule` imports `BrokersModule`
   (no cycle). Web `/analytics` adds an account-scoped "Live trade analytics" section and replaces
   the three duplicated metric grids/trade tables with shared `TradeMetricCards`/`TradesTable`
   components. No schema change. Unit tests: api **340** (**606 total**), web
   typecheck/lint/`next build` pass.
6. **Phase 9 — Increment 8: per-strategy trade analytics (2026-09-19)** — `listFilledByBotIds`
   on `PaperOrderRepository` (empty-set short-circuit, `bot_id IN` + positive fill, ascending) feeds
   new `AnalyticsService.strategyTradeAnalytics`, which derives the user's bots for a `strategyId`
   from `BotRepository.listByUserId` (ownership never trusted from the path), then rebuilds FIFO
   round trips across all accounts/symbols. New `GET /api/analytics/strategies/:strategyId/trades`
   (404 when the user runs no bot for it). Web `/analytics` gains a worldwide (not account-scoped)
   "Strategy trade analytics" section with a strategy dropdown (bot counts), metric cards and a
   recent-trades table. No schema change (reuses increment 7 attribution). Unit tests: api **335**
   (**601 total**), web typecheck/lint/`next build` pass.
7. **Phase 9 — Increment 7: per-bot trade analytics (2026-09-19)** — additive migration
...
8. **Phase 9 — Increment 6: performance report CSV export (2026-09-18)** — added
   ...
   typecheck/lint/`next build` pass.
9. **Phase 9 — Increment 5: performance report (2026-09-18)** — added
...
12. **Phase 9 — Increment 2: portfolio analytics UI (2026-09-18)** — added the `/analytics` page
...
13. **Phase 9 — Increment 1: portfolio analytics (2026-09-18)** — added the `analytics` module with
...
14. **Phase 8 — Increment 11: Binance testnet smoke-test tooling (2026-09-18)** — added
    ...
    network to testnet).
15. **Phase 8 — Increment 10: Binance fill-fee accounting (2026-09-18)** — `BinanceAdapter.getOrder`
...
16. **Phase 8 — Increment 9: Binance OCO protective brackets (2026-09-18)** — added the optional
...
17. **Phase 8 — Increment 8: Binance Testnet becomes the primary live broker (2026-09-18)** — added the
    ...
    listed).
18. **Phase 8 — Increment 7: frontend live broker monitor + order controls (2026-09-15)** — added the
...
19. **Phase 8 — Increment 6: live account monitor + position reconciliation (2026-09-15)** — added
...
20. **Phase 8 — Increment 5: live order management + emergency controls (2026-09-15)** — added
...
21. **Phase 8 — Increment 4: live bot routing + circuit breaker (2026-09-15)** — added the
...
22. **Phase 8 — Increment 3: order reconciliation engine (2026-09-15)** — added
...
23. **Phase 8 — Increment 2: API brokers module + env-backed registry (2026-09-15)** — added
...
24. **Phase 8 — Increment 1: `BybitAdapter` (2026-09-15)** — built the Bybit v5 adapter in
...
25. **Persisted per-cycle signals/orders + bot-cycle E2E (2026-09-15)** — added `bot_run_cycles`
...
26. **Fixed three latent defects surfaced by the new E2E** — (a) `MarketsService.getCandles`
...
27. **JWT-guarded the backtests API (2026-09-15)** — closed the auth inconsistency flagged in
...
**Phase 11 — next: extend the notification emitters and add an unread badge.**

1. Emit from risk/order/broker paths next (`RISK_CHECK_FAILED`, `ORDER_REJECTED`, `BROKER_ERROR`,
   `CIRCUIT_BREAKER_TRIGGERED`) so every important platform event (AGENTS.md §25) reaches the
   notification center, not just bot lifecycle.
2. Add an unread-count badge to the UI — needs a shared navbar/header component (today each page
   carries its own links), so pulling the nav into one layout component is the enabling step.
3. Separately: execute `smoke:binance` against Binance Testnet from a networked machine once available
   — this remains the only unverified live path (Phase 8); apply the pending additive migrations
   (`…011`, `…012`, `…013`) and re-run E2E once Docker/PostgreSQL/Redis are running here.
